...
if __name__ == "__main__":
    ...
    cam = cv2.VideoCapture(0)  # Use the default webcam (index 0)
    ...
    # Start reading the frames from webcam
    check, frame = cam.read()
    cv2.waitKey(2000)

    if check:
        # If able to read from webcam, start capturing frames
        while check:
            check, frame = cam.read()
            if not check:
                continue
            ...
            # Show the filter, if only 1 face is found
            if len(faces) == 1:
                # Extract face location
                x, y, w, h = faces[0]

                # Adjust the jewelry to the face's placement
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)
                fw, fh = int(w * dw), int(w * dh)
                new_impose = cv2.resize(impose, (fw, fh))

                # Superimpose the jewelry over the captured frame
